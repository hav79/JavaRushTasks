...
public class Dog implements Pet {
     ...
     * 1) из класса А скопировать логику себе (это плохо, т.к. поддерживать несколько
     *    копий одного и того же кода проблематично)
     ...
     *    использовать его методы вместе с методами и полями нашего класса,
     ...
     * Внутри метода toSayable создайте class DogPet, который наследуется от SuperDog
     * и реализует интерфейс Sayable
     * создайте метод private String getName(), который будет использовать логику
     * двух классов - Dog и SuperDog.
     * Пусть этот метод возвращает имя собаки (класс Dog), которое с обеих сторон
     * выделено getSuperQuotes (класс SuperDog)
     * Пример, " *** Барбос *** "
     * Логика метода say:
